...
kill_softprobe_pidfile() {
  ...
  if [[ -n "${pid:-}" ]] && kill -0 "$pid" 2>/dev/null; then
    ...
    if [[ "$cmd" == *softprobe-runtime* ]]; then
      echo "==> stopping Softprobe $label pid=$pid"
...
kill_softprobe_pidfile "$WRITE_PID_FILE" "write"
kill_softprobe_pidfile "$READ_PID_FILE" "read"
kill_softprobe_pidfile "$PID_FILE" "legacy"
