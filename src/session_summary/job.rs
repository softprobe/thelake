...
async fn list_summary_scopes(
    ...
    if let Some(registry) = scope_registry {
...
async fn ensure_hot_attrs_if_registry(
    scope_registry: &Option<DuckLakeScopeResolver>,
...
impl Job for SessionSummaryReduceJob {
    ...
    async fn run(&self, scope_key: &str) -> Result<()> {
        ...
        ensure_hot_attrs_if_registry(&self.scope_registry, &ducklake).await?;
        reduce_tenant(
            &self.pool,
