...
**Supersedes:** ChatGPT “Design Session Indexing” share; earlier drafts that put the directory in Explorer Supabase, dual-wrote DuckLake `session_facts`, or used a long-lived in-memory span counter
...
**Single derived store:** skinny `session_index` in **thelake catalog Postgres** (per tenant metadata schema).  
...
async job runner       → lease session_index.reduce per tenant
                       → time-scoped aggregate FROM traces
                       → UPSERT session_index; ack dirty rows
...
**Dirty write rule:** never per span — once per successful lake flush batch. `session_index.enabled` requires `ingest.flush_interval_seconds > 0` (soft coalesce).
...
1. Explorer does not ingest → thelake owns index refresh.  
2. No dual derived table in DuckLake + Postgres.  
3. Evidence only in DuckLake; Postgres holds index + dirty queue + job leases.  
...
- Index **reconstructible from `traces`**; index loss ≠ evidence loss.  
- Index numbers come from **re-aggregating DuckLake**, not a second long-lived counter.  
- Reducer is an **async leased job** (DRY with maintenance) — see [`async-jobs.md`](./async-jobs.md).  
- Simple ops: no Kafka / Redis / CH / Elastic / separate indexer.  
...
┌──────────────────────────────────────────────────────────────┐
│ Clients                                                      │
│   list  → POST /v1/llm/sessions/search  (session_index)      │
│   detail→ GET  /v1/llm/sessions/{id}…   (traces)             │
└────────────────────────────▲─────────────────────────────────┘
                             │
┌────────────────────────────┴─────────────────────────────────┐
│ thelake replicas (N)                                         │
│   ingest (all): traces commit → UPSERT session_index_dirty   │
│   async_jobs runner (all tick; Postgres lease = single winner│
│     per job_name + scope_key):                               │
│       maintenance.compact / maintenance.metadata             │
│       dropdown.prune                                         │
│       session_index.reduce / session_index.rebuild           │
│   query: list → session_index; detail → traces               │
└───────────────┬────────────────────────────▲─────────────────┘
                ▼                            │
┌───────────────────────────┐   ┌────────────┴─────────────────┐
│ DuckLake                  │   │ Catalog Postgres             │
│   traces, logs, scores    │   │   thelake_job_lease (shared) │
│   evidence only           │   │   session_index + _dirty     │
...
| List triage fields | `session_index` | Derived, rebuildable |
| Dirty session hints | `session_index_dirty` | Ephemeral work queue |
| Who runs async work | `thelake_job_lease` | Coordination only |

```text
session_index MUST be reconstructible from traces.
traces MUST NOT depend on session_index.
```

**Multi-instance / DRY:** do not give session-index its own timer or lock. It is a job on the shared runner. Today’s maintenance scheduler has **no** cross-replica lease — that gap is fixed by the same design. Details: [`async-jobs.md`](./async-jobs.md).

---

## 5. Data model

### 5.1 `session_index` (catalog Postgres / SQLite metadata)
...
CREATE INDEX session_index_recent
  ON session_index (start_time DESC, session_id);
CREATE INDEX session_index_agent
  ON session_index (agent_name, start_time DESC, session_id)
  WHERE agent_name IS NOT NULL;
CREATE INDEX session_index_errors
  ON session_index (start_time DESC, session_id)
  WHERE error_count > 0;
```

### 5.2 `session_index_dirty` (same tenant schema)
...
That is already “batched” whenever the OTLP path commits many spans together. To keep commit (and thus dirty) frequency low in multi-replica production, **session_index requires soft coalesce**:

```yaml
ingest:
  flush_interval_seconds: > 0   # required when session_index.enabled
session_index:
  enabled: true
```

Default thelake today is flush-through (`flush_interval_seconds: 0`) — fine for tiny demos, but with session_index it would dirty-UPSERT once per OTLP request. Soft coalesce merges requests into fewer DuckLake writes → fewer dirty UPSERTs. Upstream collector batching still matters; coalesce amortizes further.
...
After a successful reduce for session S, index(S) matches the lake aggregate for S over the chosen `[from,to]`.
...
| | |
|---|---|
| **Where** | `SessionIndexReduceJob` on the shared `async_jobs` runner (same process binary as maintenance) |
| **When** | On `session_index.reducer_interval_ms` (e.g. 2–5s), after winning `thelake_job_lease` for `(session_index.reduce, tenant_id)` |
...
`session_index.rebuild` is the same runner, longer interval or ops-triggered, also leased.

### 6.2 Why not RAM-only TouchSet

With multiple thelake instances, replica A’s in-memory dirty set is invisible to replica B. If only the lease holder reduces, dirty state **must** live in Postgres (`session_index_dirty`). Optional process-local coalesce before dirty UPSERT is an optimization only.
...
ingest batch (soft coalesce flush or single OTLP commit)
  ...
  └─2─► ONE batched UPSERT session_index_dirty
          for distinct session_ids in that batch
          (min_ts/max_ts folded in memory first — never per span)

SessionIndexReduceJob (lease winner only)
  ├─1─► SELECT dirty rows LIMIT N (snapshot updated_at)
  ├─2─► for each id: [from,to] per §6.5
  ├─3─► time-scoped GROUP BY FROM traces
  ├─4─► UPSERT session_index (absolute replace of aggregates)
  └─5─► DELETE dirty rows with updated_at <= snapshot
```

Config gate: `session_index.enabled` implies `ingest.flush_interval_seconds > 0` (reject or auto-enable coalesce at startup — pick one in implementation; prefer **reject** so ops is explicit).
...
```sql
INSERT INTO session_index AS s (...)
VALUES (...)
...
from = least(coalesce(session_index.start_time, dirty.min_ts), dirty.min_ts)
```

Plus `record_date BETWEEN date(from) AND date(to)`. Clamps: `max_reduce_span`, `max_sessions_per_reduce`; oversized windows chunk or defer to `session_index.rebuild`.

### 6.6 Late spans

No FINALIZED. Late span → dirty UPSERT → next leased reduce replaces the index row from `traces`.
...
`POST /v1/llm/sessions/search` → select from `session_index` (cursor on `(start_time, session_id)` desc). Steady-state path does **not** scan `traces`.

Fallback flag: legacy aggregate over `traces` for empty index / rollout only.
...
- **Done (Stage 0):** removed `sessionCountOverrides` — list uses server `SessionSummary` counts only; no window `observations/search`.  
  Until Stage 3 (`session_index`), list latency still tracks lake `sessions/search` cost; STEPS/RESULT may diverge from detail (bubbled errors / duplicate spans). Child-session folding on the server list path is gone with the scan (client-side aggregate fallback still folds); restore via index fields or server `roots_only` later.  
- Findings/agents stay in Supabase UI join by `session_id`.  
- Explorer never writes `session_index`.
...
| Truncate `session_index` | Untouched | Rebuild restores list |

Forbidden: fail ingest on index errors; store payloads in Postgres; DuckLake summary table; RAM counter as SoT for list numbers.
...
ingest:
  flush_interval_seconds: 2   # required when session_index.enabled (> 0)
session_index:
  enabled: true
...
Startup validation: if `session_index.enabled` and `ingest.flush_interval_seconds == 0`, fail config load with a clear message (force soft coalesce). Dirty UPSERT count should track **lake flush count**, not span count.

Ops: `POST /v1/llm/sessions/index/rebuild` `{from,to}` triggers leased `session_index.rebuild`; metrics for dirty depth, reducer lag, lease steal, dirty_upserts vs span_writes.
...
Checkbox task list (sequential order + **[P]** parallel marks): [`session-list-index-tasks.md`](./session-list-index-tasks.md).
...
| **1** | `session_index` + `session_index_dirty` DDL; ingest dirty UPSERT |
| **2** | `session_index.reduce` job on shared runner |
| **3** | `sessions/search` reads index |
| **4** | Leased `session_index.rebuild` (periodic + ops) |
| **5** | Promote hot list columns |

**Do not** ship a private session-index timer before stage A. Maintenance must gain leases in the same change set family.
...
| Private session-index `tokio::interval` + ad-hoc lock | Violates DRY; maintenance already needs shared leases |
| Dual maintenance lock + session-index lock | Two coordination systems |
...
Kept: disposable index, cursor paging, batched updates, evidence ≠ index.  
...
3. Truncating index leaves Parquet intact; rebuild restores list.  
4. Ingest ≠ blocked on index.  
5. No DuckLake session-summary table.  
6. No Explorer window-wide obs scan on list.  
7. After reduce(S), index(S) matches aggregate(S) on `traces` over the chosen `[from,to]`.  
8. No reducer/rebuild SQL ships without `record_date` + timestamp bounds.  
9. Session-index reduce uses the same lease module as maintenance; two replicas never dual-compact or dual-reduce one tenant.  
