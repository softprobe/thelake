...
| Query | `src/query/duckdb.rs`, `src/query/cache.rs`, `src/query/workspace_views.rs` | Query connection pool, DuckLake attach, cache setup, isolated table qualification, and shared-mode logical view policy | `QueryEngine` |
| Maintenance | `src/compaction/executor.rs`, `src/compaction/session_summary_access.rs` | `MaintenanceEngine` owns physical-scope DuckDB sessions, compaction, metadata cleanup, and the reducer/rebuild facade | `MaintenanceEngine` |
