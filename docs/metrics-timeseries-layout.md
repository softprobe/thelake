...
| Inlined catalog rows for skinny tables (AC-F7) | Default `data_inlining_row_limit` **500**; TWCS **wait-for-next-run** (merge live Parquet only; do **not** flush inlined rows every pass). Over-limit writes create Parquet for later merges; ≤limit batches may remain catalog-resident. |
...
- **2026-09-20 (inline default 500):** Softprobe `data_inlining_row_limit` default revised **10_000 → 500** (DuckLake-aligned). Large inlined chunks made session detail TABLE_SCAN pull too many Postgres rows.
- **2026-09-11 (AC-F7 wait-for-next-run):** Default `data_inlining_row_limit=10_000` (later revised to 500); maintenance no longer flushes inlined rows before TWCS. Downsample still reads the DuckLake table (inlined ∪ Parquet).
