...
> **Note (2026-09-21):** Product metrics / Prometheus / PromQL have been
> removed. Treat Prometheus oracle/harness rows below as historical audit
> evidence only; they are not a current product surface. Live compatibility is
> Loki + Tempo (+ Grafana wiring).
...
| `src/storage/schema/tables.rs` | Required for Loki/Tempo nanosecond fidelity and trace fidelity columns. | `src/storage/schema/tables.rs::tests::logs_timestamps_use_nanosecond_contract`; `tests/integration/http_api.rs`. | Logs and traces only. |
...
~~`tests/compat/support/prometheus_oracle.rs`~~ — historical Prometheus oracle
row; product Prometheus removed. Do not treat as a current requirement.
...
~~`src/compat/prometheus/mod.rs`~~ — historical; product Prometheus removed.
...
particular, `docs/compat/capability.v0.yaml`, `docs/compat/matrix.md`, and
...
implemented Loki/Tempo subset and pinned references.
