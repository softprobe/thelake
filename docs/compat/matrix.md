...
**Status:** Phase 0 baseline remains approved; Loki Phase 2 and Tempo Phase 3 repository subsets are implemented; Grafana Phase 4 repository wiring and its validation-only harness are implemented, while real service-backed acceptance requires external evidence  
**Version:** `compat.v0`  
**Last updated:** 2026-09-21

OpenTelemetry is the **canonical write path** for **traces and logs**. Loki and
Tempo compatibility is **query-only**. Write, push, remote_write, alerting, and
ruler APIs are out of scope.

**Product metrics / Prometheus / PromQL are out of scope** (removed). Process
self-monitoring may still export OTLP metrics via `OTEL_EXPORTER_OTLP_*` when
`self_monitoring.enabled` is true; that is not a customer Prometheus product.
...
path for traces and logs, compatibility routes are query-only, and shared
authentication, tenant, error, limit, and storage-fidelity rules remain the
baseline contract. Loki and Tempo add implemented discovery/query subsets;
...
Tempo, or Grafana service accepted the same workload.
...
1. Provide a Docker daemon and verify the pinned references with
   `make check-compat-reference-pins`: `grafana/loki:3.1.1`,
   `grafana/tempo:2.6.1`, and `grafana/grafana:11.2.0`.
2. Run `make test-loki-diff` and `make test-tempo-diff` against those real
   reference services. Each run must retain complete raw and normalized
   per-case evidence and have no unapproved normalized differences.
3. For Grafana, supply `GRAFANA_REFERENCE_DIGEST` matching the pinned
   ...
   passing, sanitized evidence for the Loki/Tempo Grafana gates; a mock
   selector cannot satisfy this gate.
...
Phase 4 repository work is implemented as native Loki/Tempo datasource
provisioning, dashboards, cross-signal wiring, and a mock/validation-only
system harness. Loki/Tempo provisioning and Explore smoke artifacts are
present. Manual stack: `make grafana-up` / `make grafana-down`
...
service-backed protocol gates, and passing Grafana evidence listed above.
No custom Grafana datasource plugin in initial scope.

## Canonical data fidelity (storage)

| Signal | Supported now | Explicit unsupported |
|--------|---------------|----------------------|
...
| Structured attributes | Scalars + arrays/kvlists (nested JSON in bags) + bytes (base64) | — |
| Customer metrics | — | Product metrics ingest and Prometheus query are removed |

## Language feature parity

Full LogQL / TraceQL parity is **not** claimed in v0. Each phase adds a
supported subset; everything else returns `unsupported_feature`.
