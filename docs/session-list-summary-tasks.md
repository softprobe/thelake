...
- [x] **0b.5** Delete rewrite shim (`remove-legacy-telemetry-sql-aliases`)

**Stage 0b notes:** Compilers/fixtures emit `traces`/`logs`. Legacy `union_*` rewrite shim deleted; public SQL names are registry tables only.
