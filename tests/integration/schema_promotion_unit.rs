...
fn test_schema_promotion_config_loading() {
    ...
    let promotion_config = SchemaPromotionConfig {
        traces: Some(TablePromotionConfig {
            ...
            resource_attributes: vec![softprobe_runtime::config::PromotedColumn {
    ...
    assert_eq!(
        traces.attributes[0].column_name,
        Some("user_id".to_string())
    );
    assert!(matches!(
        traces.attributes[0].data_type,
        Some(PromotedDataType::String)
    ));
...
fn test_schema_generation_with_promotion() {
    ...
    assert!(
        ...
        "user_id should be in schema"
    );
    assert!(
        fields.contains(&"department".to_string()),
        "department should be in schema"
    );
    assert!(
        fields.contains(&"service_name".to_string()),
        "service_name should be in schema"
    );
    ...
    assert!(
        fields.contains(&"record_date".to_string()),
        "record_date should still exist"
    );
}
...
fn test_schema_promotion_data_types() {
    ...
    let is_active_field = arrow_schema
        .field_with_name("is_active")
        .expect("is_active field should exist");
    assert!(matches!(
